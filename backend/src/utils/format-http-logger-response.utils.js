import redactLogData from "./redact-logs.utils.js";
...
const formatHTTPLoggerResponse = (context) => {
  ...
  if (context.req?.requestStartTime) {
    const endTime = Date.now() - context.context?.req?.requestStartTime;
  ...
  return {
    request: {
      ...
      clientIp:
        context.req?.headers[HTTPHeaders.FORWARDED_FOR] ??
        context.req?.socket.remoteAddresss,
      requestDuration,
    },
    response: {
